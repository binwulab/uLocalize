function analysisPBodyFunc(filenames, fileNumber, p_pBody, p_RNA, showRNA, displayRange, calibration)
...
outlineFile=fullfile(outlineFolder, [pBodyFile,'__outline.txt']);
...
[cell_pts_pBody, ~, cell_prop_pBody, final_pts_pBody, smooth, p_pBody, cell_granule_pBody]=uLocalizeFQ(img,p_pBody,outlineFile,resultFile, maskType, ...
...
for i=1:numel(cell_prop_RNA)
    ...
    for j=1:cc.NumObjects
        bndBox=getObjBndBox(cc.PixelIdxList{j}, cc.ImageSize, p_RNA.bg_extension);
...
%%Fitting the RNA
[cell_pts_RNA,cell_ts_RNA,cell_prop_RNA,final_pts_RNA,smooth, p_RNA, cell_granule_RNA]=uLocalizeFQ(img,p_RNA,outlineFile,resultFile, maskType, ...
    cellRegion,'filteredImage',smooth,'forceFilter',forceFilter, 'granuleCC', cell_granule_pBody, 'calibration', calibration);
