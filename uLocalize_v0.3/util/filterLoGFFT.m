...
%           LoG: the fft of the filter, saved to speed up the program
%    OUTPUT: 
%           y : LoG filtered signal
%           LoG: the fft of the filter
%
...
function [y,LoG] = filterLoGFFT(signal, sigma, LoG)
dims = ndims(signal);
...
switch dims
    ...
    case 2
        [ny,nx] = size(signal);
        if ~(nargin==3 && isequal(size(signal),size(LoG)))
