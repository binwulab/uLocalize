function [fspots2,fpix2,n_wrong,n_double] = ...
    clean_up_spots_array_3D_clean3(fpix,vox_size,nx,ny,nz,n_exclusion)
%series of checks on the detected spots to remove aberrant detections

fpix = sort_array_by_col_value(fpix,4,'descend');
...
not_wrong = logical((fpix(:,4) ~= -1).*(fpix(:,1) >= 0).*(fpix(:,1) < nx).*(fpix(:,2) >= 0 ).*(fpix(:,2) < ny ).*( fpix(:,3) >= 0 ).*( fpix(:,3) < nz)); 
...
tmp = [uint16(fpix(:,1:3)),uint16((size(fpix,1):-1:1)')];
